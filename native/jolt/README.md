...
The binding layer tracks `jrouwe/JoltPhysics.js` `1.1.0` and fetches Jolt C++
directly from `jrouwe/JoltPhysics` at `v5.6.0`. It sets
`CROSS_PLATFORM_DETERMINISTIC=ON` by default. Runtime deterministic stepping
still needs to be enabled through the wrapper API.
